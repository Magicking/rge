...
contract Deploy is Script {
    ...
    function run() public {
        ...
        ERC1967Proxy proxy =
            new ERC1967Proxy(address(rge), abi.encodeCall(ReaperGambitEpitaph.initialize, (renderer, pricer)));
...
contract UpdatePricerV0 is Script {
    ...
    function run() public {
        ...
        IPricing pricerV0 = IPricing(
            address(
                new ERC1967Proxy(address(new PricingV0()), abi.encodeCall(PricingV0.initialize, (address(RGE_ADDRESS))))
            )
        );
